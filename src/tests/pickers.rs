...
fn hotlist_state(paths: &[&str]) -> AppState {
    AppState {
        ...
        directory_hotlist: paths.iter().map(PathBuf::from).collect(),
        ..Default::default()
    }
}
...
fn hotlist_picker_delete_middle_and_last() {
    ...
    state.picker_selected = state.hotlist().len() - 1;
    pickers::handle_list_picker(&mut state, KeyCode::Char('d'));
...
fn picker_wrap_empty_and_single() {
    ...
    assert_eq!(empty.picker_selected, 0);
    ...
    assert_eq!(single.picker_selected, 0);
